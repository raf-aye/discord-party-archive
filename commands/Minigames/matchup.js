...
const moveOn = require('../../functions/Matchup/startGame.js')
...
exports.run = async (bot, message, args, prefix, games, lang) => {
  ...
  const msg = await message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description(lang.minigames.general.confirmSingle.replace("[GAME]", lang.gameTypes.matchup)))
...
collector2.on('collect', async (m, emoji, userID) => {
  ...
  if (emoji.id === "712036483694854164") {
    ...
    stats: {matches: 0},
    misc: {ownerTriedStart: 0, triedJoining: [], sentMessage: [], afkStreak: [0, 0], inLobby: false, played: 0, allowedToPlay: true},
    board: ['⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜', '⬜'],
    rawBoard: [], // This will be filled later
    currentPlayed: [],
    played: [],
    filledIndexes: [],
    msgID: ""
    ...
    msg.delete()
    collector2.stop()
    bot.executeWebhook("737839838295883846", "Uv6kTscp1imCDD0rauJTeYqRygeQdPrW90cjuEj7_gXpIRN23DUbLHH0jtfeJWUwsO_y", {embeds: [new Eris.Embed().title("New Game Started").thumbnail(bot.user.avatarURL).color(0x7289DA).timestamp().description(`**Game Type** - Matchup\n**Server** - ${message.guild.name}\n**Players**:\n\n${games.get(message.guild.id)[message.author.id].lobby.memberUsername.map((u, i) => `${games.get(message.guild.id)[message.author.id].lobby.emojis[i]} ${u}`).join('\n')}`)]})
    moveOn(bot, message, games.get(message.guild.id)[message.author.id], games, prefix, lang)
  } else if (emoji.id === "712036483967483975") {
    msg.delete()
    collector2.stop()
    message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description(lang.minigames.general.cancel.replace("[GAME]", lang.gameTypes.matchup)))
  }
});
...
exports.help = {
  name: "matchup",
  description: "Match 2 Items together!",
  usage: "matchup",
  category: "Minigames",
  emoji: "<:Matchup:735152886073393175>"
}

exports.aliases = ['startMatchup', 'match', 'startMatch']