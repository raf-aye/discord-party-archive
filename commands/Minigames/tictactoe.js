...
const moveOn = require('../../functions/Tic Tac Toe/startGame.js')
...
exports.run = async (bot, message, args, prefix, games, lang) => {
  ...
  if (!res) emoji1 = {P1: '<:Player1:714906015912689685>', P2: "<:Player2:714906015694585877>", P3: "<:Player3:714906015971278919>", P4: "<:Player4:714906015966953552>"}
    ...
    if (["one", "1"].includes(args[0].toLowerCase())) {
      const msg = await message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description(lang.minigames.general.confirmSingle.replace("[GAME]", lang.gameTypes.ttt)))
...
collector2.on('collect', async (m, emoji, userID) => {
  ...
  if (emoji.id === "712036483694854164") {
    ...
    spotsAvailable: ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'],
    spotsTaken: [],
    spots: {a: "<:Blurple_A:717617609431842846>", b: "<:Blurple_B:717617609373384765>", c: "<:Blurple_C:717617609562128425>", d: "<:Blurple_D:717617609314402306>", e: "<:Blurple_E:717617609557934150>", f: "<:Blurple_F:717617609373384776>", g: "<:Blurple_G:717617609633431603>", h: "<:Blurple_H:717617609427779594>", i: "<:Blurple_I:717617609331441685>"},
    rotation: [message.author.id],
    msgID: ""
  }

    if (!games.get(message.guild.id)) games.set(message.guild.id, gameObj)
    else Object.assign(games.get(message.guild.id), gameObj)
    bot.executeWebhook("737839838295883846", "Uv6kTscp1imCDD0rauJTeYqRygeQdPrW90cjuEj7_gXpIRN23DUbLHH0jtfeJWUwsO_y", {embeds: [new Eris.Embed().title("New Game Started").thumbnail(bot.user.avatarURL).color(0x7289DA).timestamp().description(`**Game Type** - Tic Tac Toe\n**Server** - ${message.guild.name}\n**Players**:\n\n${games.get(message.guild.id)[message.author.id].lobby.memberUsername.map((u, i) => `${games.get(message.guild.id)[message.author.id].lobby.emojis[i]} ${u}`).join('\n')}`)]})
    moveOn(bot, message, games.get(message.guild.id)[message.author.id], games, prefix, lang)
  } else if (emoji.id === "712036483967483975") {
    collector2.stop()
    msg.delete()
    message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description(lang.minigames.general.cancel.replace("[GAME]", lang.gameTypes.ttt)))
  }
});
    }
    if (["two", "2"].includes(args[0].toLowerCase())) {
      const msg = await message.channel.createEmbed(new Eris.Embed().thumbnail(bot.user.avatarURL).title(lang.minigames.general.inLobby.replace("[GAME]", lang.gameTypes.ttt)).color(0x7289DA).description(`${emoji1.P1} ${lang.players.p1} - **${message.author.username} (${lang.misc.host})**\n\n<:Player2:714906015694585877> ${lang.players.p2} - **${lang.misc.waiting}** <a:Loading:712097176313921548>\n---------------------------------------------------\n\n<:Yes:712036483694854164> - ${lang.minigames.general.join.replace("[GAME]", lang.gameTypes.ttt)}\n\n<:No:712036483967483975> - ${lang.minigames.general.leave.replace("[GAME]", lang.gameTypes.ttt)}`))
    msg.addReaction(":Yes:712036483694854164").then(() => msg.addReaction(":No:712036483967483975"))
...
collector.on('collect', async (m, emoji, userID) => {
  ...
  if (emoji2.id === "712036483694854164") {
    ...
    bot.executeWebhook("737839838295883846", "Uv6kTscp1imCDD0rauJTeYqRygeQdPrW90cjuEj7_gXpIRN23DUbLHH0jtfeJWUwsO_y", {embeds: [new Eris.Embed().title("New Game Started").thumbnail(bot.user.avatarURL).color(0x7289DA).timestamp().description(`**Game Type** - Rock Paper Scissors\n**Server** - ${message.guild.name}\n**Players**:\n\n${currentGame.lobby.memberUsername.map((u, i) => `${currentGame.lobby.emojis[i]} ${u}`).join('\n')}`)]})
...
let emoji4 = "<:Player4:714906015966953552>"
  ...
  if (currentGame.lobby.emojis[3]) emoji4 = currentGame.lobby.emojis[3].P4
    ...
    const gameInfo = {embed: {
      ...
      title: lang.minigames.general.inLobby.replace("[GAME]", lang.gameTypes.ttt),
      color: 0x7289DA
    }}
  ...
  if (currentGame.misc.confirm && currentGame.misc.confirm === 1) return;
    ...
    currentGame.status.splice(currentGame.lobby.members.indexOf(user.id), 1)
...
let emoji4 = "<:Player4:714906015966953552>"
  ...
  if (currentGame.lobby.maxPlayers === 2) msg.edit({embed: {color: 0x7289DA, thumbnail: {url: bot.user.avatarURL}, title: `${lang.minigames.general.inLobby.replace("[GAME]", lang.gameTypes.ttt)}`, description: `${emoji1.P1} ${lang.players.p1} - **${message.author.username} (${lang.misc.host})**\n\n${emoji2} ${lang.players.p2} - **${player2}**\n\n<:Yes:712036483694854164> - ${lang.minigames.general.join.replace("[GAME]", lang.gameTypes.ttt)}\n\n<:No:712036483967483975> - ${lang.minigames.general.leave.replace("[GAME]", lang.gameTypes.ttt)}`}})
  }
});
...
exports.help = {
  name: "tictactoe",
  description: "Match 3 in a row!",
  usage: "tictactoe [1 | 2]",
  category: "Minigames",
  emoji: "<:TicTacToe_X:717504662412066909>"
}

exports.aliases = ['startTictactoe', 'ttt', 'startTTT']
