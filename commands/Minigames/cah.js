...
const moveOn = require('../../functions/CAH/startGame.js')
...
if (games.get(message.guild.id) && Object.keys(games.get(message.guild.id)).length >= 10) return message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description("**There are already 10 games in this server, please wait until one ends!**"))
  ...
  if (!args[0]) return message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description(`**How many players are joining this round?**\n\nPlease specify by doing \`${prefix}ata [1 | 3 | 4 | 5 | 6]\``))
  if (!["one", "three", "four", "five", "six", "1", "3", "4", "5", "6"].includes(args[0].toLowerCase())) return message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description(`That is not a valid player size! You can only have \`1\` or \`3-6\` players in a room!\n\nCorrect usage: \`${prefix}atas [1 | 3 | 4 | 5 | 6]\``))
  ...
  if (!res) emoji1 = {P1: '<:Player1:714906015912689685>', P2: "<:Player2:714906015694585877>", P3: "<:Player3:714906015971278919>", P4: "<:Player4:714906015966953552>"}
    ...
    if (["one", "1"].includes(args[0].toLowerCase())) {
      const msg = await message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description("Are you sure you want to start the Cards Against Humanity round by yourself?"))
...
collector2.on('collect', async (m, emoji, userID) => {
  ...
  if (emoji.id === "712036483694854164") {
    ...
    bot.executeWebhook("737839838295883846", "Uv6kTscp1imCDD0rauJTeYqRygeQdPrW90cjuEj7_gXpIRN23DUbLHH0jtfeJWUwsO_y", {embeds: [new Eris.Embed().title("New Game Started").thumbnail(bot.user.avatarURL).color(0x7289DA).timestamp().description(`**Game Type** - Cards Against Humanity\n**Server** - ${message.guild.name}\n**Players**:\n\n${games.get(message.guild.id)[message.author.id].lobby.memberUsername.map((u, i) => `${games.get(message.guild.id)[message.author.id].lobby.emojis[i]} ${u}`).join('\n')}`)]})

    moveOn(bot, message, games.get(message.guild.id)[message.author.id], games, prefix)
  } else if (emoji.id === "712036483967483975") {
    collector2.stop()
    msg.delete()
    message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description("Cancelled Cards Against Humanity"))
  }
});
    }
    if (["two", "2"].includes(args[0].toLowerCase())) {
    const msg = await message.channel.createEmbed(new Eris.Embed().thumbnail(bot.user.avatarURL).title("Players in the Cards Against Humanity Lobby").color(0x7289DA).description(`${emoji1.P1} Player 1 - **${message.author.username} (Host)**\n\n<:Player2:714906015694585877> Player 2 - **Waiting...** <a:Loading:712097176313921548>\n---------------------------------------------------\n\n<:Yes:712036483694854164> - **Join the Game (Host can start the Cards Against Humanity)**\n\n<:No:712036483967483975> - **Leave the Game (Host can stop the Cards Against Humanity)**`))
    msg.addReaction(":Yes:712036483694854164").then(() => msg.addReaction(":No:712036483967483975"))
    ...
    misc: {ownerTriedStart: 0, triedJoining: [], sentMessage: [], afkStreak: [0, 0], inLobby: true, allowedToPlay: true},
    rotation: [],
    decks: [{red: [], green: []}, {red: [], green: []}, {red: [], green: []}],
    greenCard: {},
    redCards: [], 
    msgID: msg.id
  }
  ...
  if (["four", "4"].includes(args[0].toLowerCase())) {
    ...
    misc: {ownerTriedStart: 0, triedJoining: [], sentMessage: [], afkStreak: [0, 0], inLobby: true, allowedToPlay: true},
    rotation: [],
    decks: [{red: [], green: []}, {red: [], green: []}, {red: [], green: []}, {red: [], green: []}], 
    greenCard: {},
    redCards: [], 
    msgID: msg.id
  }
...
collector2.on('end', collected => {
  ...
  bruh.delete()
    ...
    if (currentGame.lobby.members[1] === "Waiting...")  {
      ...
      currentGame.lobby.memberUsername[3] = user.username
      currentGame.lobby.currentMembers++
...
const gameInfo = {embed: {
  ...
  title: "Players in the Cards Against Humanity Lobby",
  color: 0x7289DA
}}
    ...
    if (currentGame.lobby.maxPlayers === 3) gameInfo.embed.description = `${emoji1.P1} Player 1 - **${message.author.username} (Host)**\n\n${emoji2} Player 2 - **${currentGame.lobby.memberUsername[1]}**\n\n${emoji3} Player 3 - **${player3}**\n---------------------------------------------------\n\n<:Yes:712036483694854164> - **Start the Game**\n\n<:No:712036483967483975> - **Stop the Game**`
    if (currentGame.lobby.maxPlayers === 4) gameInfo.embed.description = `${emoji1.P1} Player 1 - **${message.author.username} (Host)**\n\n${emoji2} Player 2 - **${currentGame.lobby.memberUsername[1]}**\n\n${emoji3} Player 3 - **${player3}**\n\n${emoji4} Player 4 - **${player4}**\n---------------------------------------------------\n\n<:Yes:712036483694854164> - **Start the Game**\n\n<:No:712036483967483975> - **Stop the Game**`
...
let emoji4 = "<:Player4:714906015966953552>"
  ...
  if (currentGame.lobby.emojis[3]) emoji4 = currentGame.lobby.emojis[3].P4
    ...
    if (currentGame.lobby.maxPlayers === 2) msg.edit({embed: {color: 0x7289DA, thumbnail: {url: bot.user.avatarURL}, title: "Players in the Cards Against Humanity Lobby", description: `${emoji1.P1} Player 1 - **${message.author.username} (Host)**\n\n${emoji2} Player 2 - **${player2}**\n\n<:Yes:712036483694854164> - **Join the Game (Host can start the Cards Against Humanity)**\n\n<:No:712036483967483975> - **Leave the Game (Host can stop the Cards Against Humanity)**`}})
    else  if (currentGame.lobby.maxPlayers === 3) msg.edit({embed: {color: 0x7289DA, thumbnail: {url: bot.user.avatarURL}, title: "Players in the Cards Against Humanity Lobby", description:`${emoji1.P1} Player 1 - **${message.author.username} (Host)**\n\n${emoji2} Player 2 - **${player2}**\n\n${emoji3} Player 3 - **${player3}**\n\n<:Yes:712036483694854164> - **Join the Game (Host can start the Cards Against Humanity)**\n\n<:No:712036483967483975> - **Leave the Game (Host can stop the Cards Against Humanity)**`}})
    else if (currentGame.lobby.maxPlayers === 4) msg.edit({embed: {color: 0x7289DA, thumbnail: {url: bot.user.avatarURL}, title: "Players in the Cards Against Humanity Lobby", description:`${emoji1.P1} Player 1 - **${message.author.username} (Host)**\n\n${emoji2} Player 2 - **${player2}**\n\n${emoji3} Player 3 - **${player3}**\n\n${emoji4} Player 4 - **${player4}**\n\n<:Yes:712036483694854164> - **Join the Game (Host can start the Cards Against Humanity)**\n\n<:No:712036483967483975> - **Leave the Game (Host can stop the Cards Against Humanity)**`}})
  }
});
...
exports.help = {
  ...
  usage: "applestoapples [1 | 3 | 4 | 5 | 6]",
  category: "Minigames",
  emoji: "🍎"
}

exports.aliases = ['startATA', "startApplestoApples", "ata"]