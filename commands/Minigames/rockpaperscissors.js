...
const moveOn = require('../../functions/Rock Paper Scissors/startGame.js')
...
exports.run = async (bot, message, args, prefix, games, lang) => {
  ...
  if (!res) emoji1 = {P1: '<:Player1:714906015912689685>', P2: "<:Player2:714906015694585877>", P3: "<:Player3:714906015971278919>", P4: "<:Player4:714906015966953552>"}
    ...
    if (["one", "1"].includes(args[0].toLowerCase())) {
      const msg = await message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description(lang.minigames.general.confirmSingle.replace("[GAME]", lang.gameTypes.rps)))
...
collector2.on('collect', async (m, emoji, userID) => {
  ...
  if (emoji.id === "712036483694854164") {
    ...
    stats: [{chosen: "", player: "P1"}, {chosen: "", player: "CPU"}],
    misc: {ownerTriedStart: 0, triedJoining: [], sentMessage: [], afkStreak: [0, 0], time: 20, inLobby: false},
    played: [],
    status: [`${emoji1.P1} **${message.author.username}** ${lang.misc.waiting} <a:Loading:712097176313921548>`],
    wins: [{wins: 0, player: "P1"}, {wins: 0, player: "CPU"}],
    msgID: []
  }
    ...
    collector2.stop()
    msg.delete()
    message.channel.createEmbed(new Eris.Embed().color(0x7289DA).description(lang.minigames.general.cancel.replace("[GAME]", lang.gameTypes.rps)))
  }
});
    }
    if (["two", "2"].includes(args[0].toLowerCase())) {
      const msg = await message.channel.createEmbed(new Eris.Embed().thumbnail(bot.user.avatarURL).title(lang.minigames.general.inLobby.replace("[GAME]", lang.gameTypes.rps)).color(0x7289DA).description(`${emoji1.P1} ${lang.players.p1} - **${message.author.username} (${lang.misc.host})**\n\n<:Player2:714906015694585877> ${lang.players.p2} - **${lang.misc.waiting}** <a:Loading:712097176313921548>\n---------------------------------------------------\n\n<:Yes:712036483694854164> - ${lang.minigames.general.join.replace("[GAME]", lang.gameTypes.rps)}\n\n<:No:712036483967483975> - ${lang.minigames.general.leave.replace("[GAME]", lang.gameTypes.rps)}`))
      msg.addReaction(":Yes:712036483694854164").then(() => msg.addReaction(":No:712036483967483975"))
    ...
    gameObj[message.author.id] = {
      ...
      channel: message.channel.id,
      stats: [{chosen: "", player: "P1"}, {chosen: "", player: "P2"}],
      misc: {ownerTriedStart: 0, triedJoining: [], sentMessage: [], afkStreak: [0, 0], time: 20, inLobby: true},
      played: [],
      status: [`${emoji1.P1} **${message.author.username}** ${lang.misc.waiting} <a:Loading:712097176313921548>`],
      wins: [{wins: 0, player: "P1"}, {wins: 0, player: "P2"}],
      msgID: msg.id
    }
...
let emoji4 = "<:Player4:714906015966953552>"
  ...
  currentGame.status.push(`${emoji2} **${currentGame.lobby.memberUsername[1]}** ${lang.misc.waiting} <a:Loading:712097176313921548>`)
      
    const gameInfo = {embed: {
      thumbnail: {url: bot.user.avatarURL},
      title: lang.minigames.general.inLobby.replace("[GAME]", lang.gameTypes.rps),
      color: 0x7289DA
    }}
...
let emoji4 = "<:Player4:714906015966953552>"
  ...
  if (currentGame.lobby.maxPlayers === 2) msg.edit({embed: {color: 0x7289DA, thumbnail: {url: bot.user.avatarURL}, title: `${lang.minigames.general.inLobby.replace("[GAME]", lang.gameTypes.rps)}`, description: `${emoji1.P1} ${lang.players.p1} - **${message.author.username} (${lang.misc.host})**\n\n${emoji2} ${lang.players.p2} - **${player2}**\n\n<:Yes:712036483694854164> - ${lang.minigames.general.join.replace("[GAME]", lang.gameTypes.rps)}\n\n<:No:712036483967483975> - ${lang.minigames.general.leave.replace("[GAME]", lang.gameTypes.rps)}`}})
  }
});
...
exports.help = {
  ...
  usage: "rps [1 | 2]",
  category: "Minigames",
  emoji: "<:Rock:721973835804573697>"
}

exports.aliases = ['startRockPaperScissors', 'rockPaperScissors', 'startRPS']
