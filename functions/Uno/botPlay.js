...
module.exports = (bot, dm, currentGame, games, prefix, message) => {
  function createEmbed(embed) {dm.createEmbed(embed)}
  ...
  let collector = new MessageCollector(bot, dm, filter, {
  time: 1200000
  });
  ...
  const cooldown = new Set();
   ...
   if (!games.get(message.guild.id) || !games.get(message.guild.id)[message.author.id]) return collector.stop()
   ...
   if (ranCommand[0] === "cards") {
     let displayCards = []
     currentGame.decks[0].forEach((c, i) => {
       if (i === 13) return displayCards.push(`... *and ${currentGame.decks[0].length - 13} more...*`)
       if (i >= 14) return;
     ...
     return dm.createEmbed(new Eris.Embed().title("Your Cards").color(0x7289DA).thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1").description(`${displayCards.join("\n")}`))
   }
  
  if (ranCommand[0] === "help") {
    return dm.createEmbed(new Eris.Embed().title("Help").color(0x7289DA).thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1").description("`play` - Lets you play a card\n`draw` - Lets you draw a card\n\n`help` - Sends this command\n`cards` - Lists your cards").footer("Remember, these commands have no prefix!"))
  }
     
   if (currentGame.rotation[0] !== m.author.id) return errorEmbed("It is not your turn yet!", m.channel)
     
   if (ranCommand[0] === "draw") {
    if (currentGame.decks[0].length >= 20) {
      createEmbed(new Eris.Embed().color(0x7289DA).description(`**${m.author.username}** tried to draw a card, but they have too many cards, so their turn will be skipped`))
    let moveOn = currentGame.rotation[0]
     currentGame.rotation.shift()
     currentGame.rotation.push(moveOn)
    return setTimeout(() => {playCard("Draw", ranCommand, "Draw", 0, collector)}, 2000)  
    }
    var rand = cards[Math.floor(Math.random() * cards.length)];
    currentGame.decks[0].push(rand)
    ...
    dm.createEmbed(new Eris.Embed().color(0x7289DA).description(`You drew a **${wordedCard}**`))
    let moveOn = currentGame.rotation[0]
     currentGame.rotation.shift()
     currentGame.rotation.push(moveOn)
     currentGame.misc.allowedToPlay = false
    return setTimeout(() => {playCard("Draw", ranCommand, "Draw", 0)}, 2000)  
   }
   ...
   if (ranCommand[0] === "wild") {
     ...
     return setTimeout(() => {playCard(cardPlayed, ranCommand, index, 0, collector)}, 2000)
   }
    ...
    if (currentGame.decks[1].length >= 20) {
      createEmbed(new Eris.Embed().color(0x7289DA).description(`**${m.author.username}** tried to do a +4, but **${currentGame.lobby.members[1]}** has too many cards, so their turn will be skipped`))
    let moveOn = currentGame.rotation[0]
     currentGame.rotation.shift()
     currentGame.rotation.push(moveOn)
     currentGame.misc.allowedToPlay = false
    ...
    newCardNames.push(masks2[r])
     ...
     createEmbed(new Eris.Embed().color(0x7289DA).description(`**${m.author.username}** has played a Wild +4 and has picked **${ranCommand[1]}**, **${currentGame.lobby.members[1]}** has to draw 4 cards!`))
   ...
   if (ranCommand[1] === "skip") {
     ...
     createEmbed(new Eris.Embed().color(0x7289DA).description(`**${m.author.username}** played a **${ranCommand[0]} Skip**!, **${currentGame.lobby.members[1]}** has been skipped!`))
     let moveOn = currentGame.rotation[0]
     currentGame.rotation.shift()
     currentGame.rotation.push(moveOn)
    ...
    if (ranCommand[1] === "reverse") {
     ...
     createEmbed(new Eris.Embed().color(0x7289DA).description(`**${m.author.username}** played a **${ranCommand[0]} Reverse**!, The playing has been reversed`))
    ...
    if (ranCommand[1] === "+2") {
      if (currentGame.decks[1].length >= 20) {
      createEmbed(new Eris.Embed().color(0x7289DA).description(`**${m.author.username}** tried to do a +2, but **${currentGame.lobby.members[1]}** has too many cards, so their turn will be skipped`))
    let moveOn = currentGame.rotation[0]
     currentGame.rotation.shift()
     currentGame.rotation.push(moveOn)
    currentGame.misc.allowedToPlay = false
   ...
   createEmbed(new Eris.Embed().color(0x7289DA).description(`**${m.author.username}** played a **${ranCommand[0]} +2**!, **${currentGame.lobby.members[1]}** has to draw 2 cards!`))     
     let moveOn = currentGame.rotation[0]
     currentGame.rotation.shift()
     currentGame.rotation.shift()
     currentGame.rotation.push(moveOn)
  ...
  collector.on('end', () => {
    ...
    dm.createEmbed(new Eris.Embed().color(0x7289DA).description("The game has ended because the game has taken longer than **20 Minutes**"))
  ...
  function playCard(cardPlayed, ranCommand, index, l, collector) {
   ...
   currentGame.decks[l].splice(index, 1)
    ...
    if (currentGame.decks[0].length === 0) return endGame(collector, false)
   ...
   currentGame.decks.forEach((deck, i) => {
     ...
     deck.forEach((d, h) => {
       if (h === 12) return hiddenDecks1.push(`... *and ${deck.length - 12} more*`)
       if (h > 12) return;
     ...
     if (i === 0) hiddenDecks2.push(`${emoji} **${bot.users.get(currentGame.lobby.members[i]).username}**'s cards:\n${hiddenDecks1.join(" ")}`)
     else hiddenDecks2.push(`<:CPU:723452091376467968> **${currentGame.lobby.members[1]} (CPU)**'s cards:\n${hiddenDecks1.join(" ")}`)
     hiddenDecks1 = []
   })
    let played = masks2[cardPlayed]
    if (cardPlayed === "Draw") played = "No Card"
    if (!["+4", "Skip", "+2"].includes(played.split(" ")[1])) {
    dm.createMessage({content: `${currentGame.decks[0].join(" ")}`, embed: new Eris.Embed().color(0x7289DA).title("Uno").thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1").description(`<:CPU:723452091376467968> **${currentGame.lobby.members[1]}**'s turn... Please wait... <a:Loading:712097176313921548>\n\n${hiddenDecks2.join("\n\n")}\n\nCurrent Card **(${masks2[currentGame.current.card]})**:`).image(images[currentGame.current.card]).footer("Forgot the commands? type \"help\" to see the commands (No Prefix!)")})
    setTimeout(() => {botPlay()}, 2000)
  } else {
    currentGame.misc.allowedToPlay = true
    dm.createMessage({content: `${currentGame.decks[0].join(" ")}`, embed: new Eris.Embed().color(0x7289DA).title("Uno").thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1").description(`${currentGame.lobby.emojis[0]} It's your turn! \n\n${hiddenDecks2.join("\n\n")}\n\nCurrent Card **(${masks2[currentGame.current.card]})**:`).image(images[currentGame.current.card]).footer("Forgot the commands? type \"help\" to see the commands (No Prefix!)")})
   }
  }
  ...
  function botPlay() {
    ...
    let skipped = false
    currentGame.decks[1].forEach((c, d) => {
      if (playedCard === true) return
      
      let card = c.split("").slice(2).slice(0, 2).join("")
      if (c.includes("A2")) card = c.split("").slice(2).slice(0, 3).join("")
      let actualCard = masks2[card].split(" ")
      
      if (actualCard[0] !== currentGame.current.color && actualCard[1] !== currentGame.current.number && !['Wild'].includes(actualCard[0])) return;
      
      let colors = ["Red", "Blue", "Green", "Yellow"]
      
      if ("Wild" === actualCard[0]) {  
    ...
    if (actualCard[1] === "+4") {
      ...
      namedDeck.forEach((card, i) => {
       if (card.includes("A2")) card = card.split("").slice(2).slice(0, 3).join("")
       else card = card.split("").slice(2).slice(0, 2).join("")
       if (i === 3) namedDeck2.push(`and ${masks2[card]}`)
       else namedDeck2.push(masks2[card])
      })
      dm.createEmbed(new Eris.Embed().color(0x7289DA).description(`**${currentGame.lobby.members[1]}** played a Wild +4, and picked **${pickedColor}**\nYou drew a ${namedDeck2.join(", ")}`))
      currentGame.current.card = "W4"
      currentGame.current.color = pickedColor
      currentGame.current.number = "Wild"

      currentGame.decks[1].splice(d, 1)
      skipped = true
      return playedCard = true
    } else {
      dm.createEmbed(new Eris.Embed().color(0x7289DA).description(`**${currentGame.lobby.members[1]}** played a Wild Card. The color is now **${pickedColor}**`))
      currentGame.current.card = "WC"
      currentGame.current.color = pickedColor
      currentGame.current.number = "Wild"
      ...
      if (actualCard[1] === "Skip") {
      dm.createEmbed(new Eris.Embed().color(0x7289DA).description(`**${currentGame.lobby.members[1]}** played a **${actualCard.join(" ")}**! Your turn has been skipped!`))
      currentGame.current.card = card
      currentGame.current.color = actualCard[0]
      currentGame.current.number = actualCard[1]
      ...
      dm.createEmbed(new Eris.Embed().color(0x7289DA).description(`**${currentGame.lobby.members[1]}** played a **${actualCard.join(" ")}**\nYou drew a ${namedDeck2.join(", ")}`))
        
      currentGame.current.card = card
      currentGame.current.color = actualCard[0]
      currentGame.current.number = "+2"
      ...
      if (actualCard[1] === "Reverse") dm.createEmbed(new Eris.Embed().color(0x7289DA).description(`**${currentGame.lobby.members[1]}** played a **${actualCard.join(" ")}**! The playing order has been reversed!`))
      else dm.createEmbed(new Eris.Embed().color(0x7289DA).description(`**${currentGame.lobby.members[1]}** played a  **${actualCard.join(" ")}**`))
      currentGame.current.card = card
      currentGame.current.color = actualCard[0]
      currentGame.current.number = actualCard[1]
    ...
    if (playedCard === false) {
      dm.createEmbed(new Eris.Embed().color(0x7289DA).description(`**${currentGame.lobby.members[1]}** had no cards to play... They drew a card`))
      currentGame.decks[1].push(cards[Math.floor(Math.random() * cards.length)])
    }
    if (currentGame.decks[1].length === 0) return endGame(collector, true)
    if (skipped === true) return setTimeout(() => {botPlay()}, 2000)
    setTimeout(() => {botMoveOn()}, 2000)
  }
  ...
  function endGame(collector, botWon) {
    currentGame.misc.allowedToPlay = false
    ...
    currentGame.decks.forEach((deck, i) => {
      let emoji;
      if (i === 1) emoji = "<:CPU:723452091376467968>"
      else emoji = currentGame.lobby.emojis[0]
      
      let user;
      if (i === 1) user = "CPU"
      else user = message.author.username
       if (deck.length === 0) return cards.push(`${emoji} ${user}'s Cards - **Empty (Winner!)**`)
       else cards.push(`${emoji} ${user}'s Cards - ${deck.join(" ")}`)
    })
    end.description = cards.join('\n')
    let addTokens = 30
    
    if (botWon === true) {
      end.field("The Bot Won... (+10 Tokens)", `\u200b`)
      addTokens = 15
    } else {
      end.field("You Won!", `\u200b`)
      addTokens = 30
    ...
    require('../../models/user.js').findOne({id: message.author.id}, (err, res) => {
      if (!res) require('../createProfile.js')(bot, message.author.id, message.author.username, addTokens, {connect4: 0, quiz: 0, rps: 0, ttt: 0, uno: 0}, {connect4: 0, hangman: 0, matchup: 0, quiz: 0, rps: 0, ttt: 0, uno: 1}, [{players: message.author.username, cards: currentGame.decks[0], game: "Uno"}], "Uno")
      else if (res) require('../addTokens.js')(message.author.id, res, addTokens, 0, {players: currentGame.lobby.memberUsername, cards: currentGame.decks[0], game: "Uno"},  "uno")
    });
    if (Object.keys(games.get(message.guild.id)).length === 1) games.delete(message.guild.id)
    else delete games.get(message.guild.id)[message.author.id]
    collector.stop()
