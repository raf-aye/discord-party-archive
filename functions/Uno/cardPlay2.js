...
module.exports = (bot, users, currentGame, games, prefix, message, lang) => {
  ...
  async function skip(dm, collect, user, mes, l, playedCard) {
    ...
    nextPlayer.dm.createEmbed(new Eris.Embed().color(0x7289DA).thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1").description(lang.minigames.uno.skipped.replace("[USER]", users.filter(u => u.id === currentGame.rotation[0])[0].username)))
    ...
    if (currentGame.rotation.length === 2) {
        ...
        users[users.findIndex(e => e.id === currentGame.rotation[0])].msg = dmMsg
         ...
         const dm = await bot.getDMChannel(currentGame.rotation[0])
         const dmMsg = await dm.createEmbed(new Eris.Embed().color(0x7289DA).title(lang.minigames.uno.gameName).description(lang.minigames.uno.sitBack).footer(lang.minigames.uno.solicited.replace("[USER]", currentGame.lobby.memberUsername[users.findIndex(e => e.id === currentGame.rotation[0])])))
         users[users.findIndex(e => e.id === currentGame.rotation[0])].msg = dmMsg
        }
  ...
  async function add2(dm, collect, user, mes, l, playedCard) {
    ...
    nextPlayer.dm.createEmbed(new Eris.Embed().color(0x7289DA).thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1").description(lang.minigames.uno.add2.replace('[USER]', users.filter(u => u.id === currentGame.rotation[0])[0].username)))
    ...
    if (currentGame.rotation.length === 2) {
        ...
        users[users.findIndex(e => e.id === currentGame.rotation[0])].msg = dmMsg
         ...
         const dm = await bot.getDMChannel(currentGame.rotation[0])
         const dmMsg = await dm.createEmbed(new Eris.Embed().color(0x7289DA).title("Uno").description(lang.minigames.uno.sitBack).thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1").footer(lang.minigames.uno.solicited.replace("[USER]", currentGame.lobby.memberUsername[users.findIndex(e => e.id === currentGame.rotation[0])])))
         users[users.findIndex(e => e.id === currentGame.rotation[0])].msg = dmMsg
        }
      currentGame.rotation.push(currentGame.rotation[0])
      currentGame.rotation.shift()
  ...
  async function wildCard(dm, collect, user, mes, l, playedCard) {
          ...
          const confirmColor = await dm.createEmbed(new Eris.Embed().color(0x7289DA).title("Uno").description(`${lang.minigames.uno.selectColor}\n<:Player2:714906015694585877> - ${lang.minigames.uno.colors.red}\n\n<:Player1:714906015912689685> - ${lang.minigames.uno.colors.blue}\n\n<:Player3:714906015971278919> - ${lang.minigames.uno.colors.green}\n\n<:Player4:714906015966953552> - ${lang.minigames.uno.colors.yellow}`))
          ...
          wildCollector.on("collect", async (wM, wEmoji, wUID) => {
            ...
            if (playedCard[1] === "4") {
              ...
              nextPlayer.dm.createEmbed(new Eris.Embed().color(0x7289DA).thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1").description(lang.minigames.uno.add4.replace("[USER]", users.filter(u => u.id === currentGame.rotation[0])[0].username).replace("[COLOR]", lang.minigames.uno.colors[color.toLowerCase()])))
              if (currentGame.rotation.length === 2) {
              const dmMsg = await dm.createMessage("temp")
              users[users.findIndex(e => e.id === currentGame.rotation[0])].msg = dmMsg
              } else {
                const dm = await bot.getDMChannel(currentGame.rotation[0])
                const dmMsg = await dm.createEmbed(new Eris.Embed().color(0x7289DA).title("Uno").description(lang.minigames.uno.sitBack).thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1").footer(lang.minigames.uno.solicited.replace("[USER]", currentGame.lobby.memberUsername[m])))
                users[users.findIndex(e => e.id === currentGame.rotation[0])].msg = dmMsg
              }
              currentGame.rotation.push(currentGame.rotation[0])
              currentGame.rotation.shift()
...
function skipPlay() {
  ...
  currentGame.decks[i].forEach(card => {
   if (card[2] === currentGame.current.color[0].toUpperCase() || card[3] === currentGame.current.number || card[2] === "W") playableCards.push({emoji: card, name: masks2[cardNames.filter(e => card.includes(e))[0]], react: reactionMasks[card], id: cardID.filter(e => card.includes(e))[0] })
  })
...
currentGame.playableCards[i] = playableCards.map(e => e.id)
        ...
        const nextMsg = await nextPlayer.msg.edit({content: currentGame.decks[i].join(" "), embed: new Eris.Embed().color(0x7289DA).title("Uno").thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1").description(`${lang.minigames.uno.description2}\n\n${lang.minigames.uno.playableCards}\n\n${playableCards.map(c => `${c.emoji} - **${c.name}**`).join('\n')}\n\n${lang.minigames.uno.inOrder}\n\n${lang.minigames.uno.currentCard2.replace("[CARD]", `${lang.minigames.uno.colors[currentGame.current.color.toLowerCase()]} ${currentGame.current.number}`)}`).image(currentGame.current.url).footer(lang.minigames.uno.solicited.replace("[USER]", currentGame.lobby.memberUsername[users.findIndex(e => e.id === currentGame.rotation[0])]))})
     ...
     hiddenDecks2.push(`${emoji} ${lang.minigames.uno.cards.replace("[USER]", bot.users.get(currentGame.lobby.members[i]).username)} ${player}\n${hiddenDecks1.join(" ")}`)
     hiddenDecks1 = []
     });
        ...
        .description(`${lang.minigames.uno.lobbyMsg}\n\n${hiddenDecks2.join("\n\n")}\n\n${lang.minigames.uno.currentCard2.replace("[CARD]", `${lang.minigames.uno.colors[currentGame.current.color.toLowerCase()]} ${currentGame.current.number}`)}`)
         const ms = await message.channel.createMessage({content: lang.minigames.uno.yourTurn.replace("[USER]", `<@${currentGame.rotation[0]}>`), embed: embed})
  ...
  async function reverse(dm, collect, user, mes, l, playedCard) {
   ...
   async function winner() {
    ...
    .color(0x7289DA)
    .thumbnail("https://cdn.discordapp.com/emojis/715385885222240277.png?v=1")
    .title(lang.minigames.uno.gameOver)
    
    let cards = []
    currentGame.lobby.memberUsername.forEach((member, i) => {

       if (currentGame.decks[i].length === 0) return cards.push(`${currentGame.lobby.emojis[i]} ${lang.minigames.uno.cards.replace("**[USER]**", member)} - **${lang.minigames.uno.winner}**`)
       else cards.push(`${currentGame.lobby.emojis[i]} ${lang.minigames.uno.cards.replace("**[USER]**", member).replace(":", "")} - ${currentGame.decks[i].join(" ")}`)
    })
    ...
    currentGame.decks.forEach((deck, i) => {
      if (i === 0)  end.field(`🏆 **${deck.user}** - (+${addTokens[i]} ${lang.misc.tokenCap})`, `\u200b`)
      if (i === 1) end.field(`🥈 **${deck.user}** - (+${addTokens[i]} ${lang.misc.tokenCap})`, `\u200b`)
      if (i === 2) end.field(`🥉 **${deck.user}** - (+${addTokens[i]} ${lang.misc.tokenCap})`, `\u200b`)
      if (i === 3) end.field(`💢 **${deck.user}** - (+${addTokens[i]} ${lang.misc.tokenCap})`, `\u200b`)
